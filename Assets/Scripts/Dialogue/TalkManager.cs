...
public class TalkManager : MonoBehaviour
{
    ...
    private void GenerateData()         // 기본/퀘스트 대사 추가
    {
        ...
        talkData.Add(10 + 2000, new string[] { "임무다. 후드.",
            ...
            "머릿수는 적지만, 총으로 무장하고 있어서 이미 마을 사람 몇 명은 죽었다고 한다.",
        ...
        talkData.Add(20 + 2000, new string[] { "정말 거점을 만들었나보군.",
            ...
            "놈들이 사용한 어떤것이라도 상관없어. 물증이 생기면 바로 공격한다.",
            "아직 놈들을 마주치지는 말고. 흔적을 모두 찾으면 보고해." });
        ...
        talkData.Add(30 + 2000, new string[] { "약물까지 사용하다니 놈들의 몰골이 궁금하구만.",
            ...
            "자리를 비운 틈을 타서 질서 없이 돌아다니는 멍청한 부하놈들의 머릿수를 줄이고 와."});

        talkData.Add(31 + 2000, new string[] { "얼른 출발해." });

        //4번 퀘스트 : 말머리 패거리 처단
        talkData.Add(40 + 2000, new string[] { "좋은 결과다. 놈들의 머릿수가 많이 줄었어.",
            "이번에 들어온 정보에 의하면 말머리도 좋은 장비는 없다고 한다.",
            "때가 됐다. 가서 패거리를 다 죽이고 와.",});
        talkData.Add(41 + 2000, new string[] { "이번 임무만 성공하면 숲이 당분간 조용하겠어." });
        ...
        talkData.Add(3000, new string[] { "이번 임무는 숲인가요, 후드?",
            ...
            "체력 포션은 빨간약초 2개, 에너지 포션은 파란 약초 2개가 필요해요.",});
